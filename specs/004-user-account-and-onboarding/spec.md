...
| Forgot password flow | Basic password reset via Supabase Auth; user requests a reset email from the login screen (OQ-ACCOUNT-004 resolved) |
...
- Email verification is not required before first MVP usage. Users may access authenticated features immediately after sign-up. Email verification may be enabled before a wider public launch if needed, but must not block the beta experience (OQ-ACCOUNT-002 resolved).
...
**Priority:** Must Have
**Source:** PRD 13.3 FR-AUTH-001; ADR-004; OQ-ACCOUNT-002 resolved
...
When an unauthenticated user attempts to access a protected feature, they must see a clear, context-specific login/sign-up prompt.

- The prompt must explain clearly that login or account creation is required for the specific feature being accessed
...
- The approved copy patterns for each context are as follows (OQ-ACCOUNT-003 resolved):

| Context | Approved Prompt Copy |
|---|---|
| AI Tutor | "Log in or create an account to use the AI Tutor. This helps us keep AI usage tied to your learning account." |
| Protected lesson (beyond Lesson 1) | "Log in or create an account to continue this lesson and save your progress." |
| Dashboard | "Log in or create an account to view your dashboard and continue learning." |
| First lesson sign-up CTA | "Create a free account to save your progress, continue the IBM i Fundamentals path, and access the AI Tutor." |

**Priority:** Must Have
**Source:** PRD 13.4 FR-ONB-005; D-PROD-005; US-ACCOUNT-007, US-ACCOUNT-009; OQ-ACCOUNT-003 resolved

---

### ACCOUNT-FR-015 — Post-Login Redirect Behavior

After a successful login or sign-up, the user must be directed to the appropriate page based on their context (OQ-ACCOUNT-001 resolved).

**Login redirect rules:**
- If the user logged in after being redirected from a protected page, they must be redirected back to the original protected page
- If the user logged in directly without a redirect context, they must be taken to the dashboard

**Sign-up redirect rules:**
- If the user signed up after trying to access a protected page, they must be shown the onboarding question first, then redirected to the original protected page
- If the user signed up directly without a redirect context, they must be shown the onboarding question first, then redirected to the dashboard

**Examples:**
| User action | Result |
|---|---|
| Protected lesson → login | Redirected to same protected lesson |
| AI Tutor → login | Redirected to AI Tutor |
| Dashboard → login | Redirected to Dashboard |
| Direct login (no redirect context) | Redirected to Dashboard |
| Protected lesson → sign-up | Onboarding → same protected lesson |
| Direct sign-up (no redirect context) | Onboarding → Dashboard |

**Priority:** Must Have
**Source:** US-ACCOUNT-002, US-ACCOUNT-006; OQ-ACCOUNT-001 resolved
...
### ACCOUNT-FR-018 — Forgot Password

The product must provide a basic forgot password flow using Supabase Auth (OQ-ACCOUNT-004 resolved).

- A "Forgot password?" link must be visible on the login screen
- Clicking the link must take the user to a simple screen where they can enter their email address
- On submission, Supabase Auth must send a password reset email to the provided address
- A user-friendly confirmation message must be shown after submission (e.g., "If an account exists for that email, a password reset link has been sent")
- The confirmation message must be non-specific to avoid revealing whether an email is registered
- The password reset link and token handling must be managed entirely by Supabase Auth; the application must not implement custom password reset token logic
- After clicking the reset link in the email, Supabase Auth must handle the password update flow
- If the email is not delivered or the link expires, the user must be able to request a new reset email

**Priority:** Must Have
**Source:** ADR-004; OQ-ACCOUNT-004 resolved
...
- A visible "Forgot password?" link that initiates the Supabase Auth password reset flow (ACCOUNT-FR-018)
...
- When an unauthenticated user encounters a protected feature, a prompt must appear in place of the protected content
- The prompt must use the approved context-specific copy (see ACCOUNT-FR-014 table)
- Call-to-action options: Log In, Create Account
- The prompt must not expose any protected content

### Forgot Password Screen

- A simple screen reachable from the login screen via the "Forgot password?" link
- An email input field and a submit button
- A non-specific confirmation message after submission (e.g., "If an account exists for that email, a password reset link has been sent")
- A link back to the login screen

### Post-Login Redirect Behavior

- After successful login, the user is taken to the original protected page they were trying to access, or to the dashboard if they logged in directly (ACCOUNT-FR-015)
- After successful sign-up, the user is taken to the onboarding question, then to the original protected page or to the dashboard (ACCOUNT-FR-015)
...
- [ ] After login, the user is directed to the original protected page they were trying to access, or to the dashboard if they logged in directly
- [ ] After sign-up, the user is directed to the onboarding question, then to the original protected page or to the dashboard
...
- [ ] A user can enter their email and request a password reset
- [ ] A non-specific confirmation message is shown after submission
- [ ] Supabase Auth sends a reset email if the address is registered
- [ ] No custom password reset token logic is implemented in application code
...
| Email deliverability — forgot password reset emails may not be received due to Supabase email configuration | Low | Medium | Test password reset email delivery before beta launch; confirm Supabase email provider settings are correct. Note: email verification is not required for MVP usage (OQ-ACCOUNT-002 resolved), so the only email dependency is the forgot password flow. |
...
- [ ] Centralized protected route middleware design is reviewed before individual routes are implemented
- [ ] Supabase email configuration is confirmed working for the forgot password reset flow before beta launch
...
| 2026-07-01 | 0.2 | Cleanup after review; resolved redirect, email verification, login prompt copy, and forgot password decisions |
| 2026-07-01 | 1.0 | Approved User Account and Onboarding SDD spec for implementation planning |
